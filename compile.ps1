...
if ($BuildOnly -or $GenerateBuild) {
    ...
    # Ensure the exp directory exists and is seeded for the target configuration.
    # .EXP files are only generated by Clarion when a project is new or its target
    # type changes - they are NOT regenerated on every build. On a clean workspace
    # we must seed from the other configuration's exp dir (exports are identical).
    $releaseExpDir = Join-Path $solutionDir "genfiles\release\exp"
    $debugExpDir   = Join-Path $solutionDir "genfiles\debug\exp"
    if ($Configuration -eq 'Debug' -and -not (Test-Path $debugExpDir)) {
        ...
        if (Test-Path $releaseExpDir) {
            ...
            Write-Host "Seeded genfiles\debug\exp from release\exp" -ForegroundColor Cyan
        }
    } elseif ($Configuration -eq 'Release' -and -not (Test-Path $releaseExpDir)) {
        New-Item -ItemType Directory -Path $releaseExpDir -Force | Out-Null
        if (Test-Path $debugExpDir) {
            Copy-Item "$debugExpDir\*" $releaseExpDir -Force
            Write-Host "Seeded genfiles\release\exp from debug\exp" -ForegroundColor Cyan
        }
    }
